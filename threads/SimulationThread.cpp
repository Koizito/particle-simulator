...
void SimulationThread::waitForStartSignal() {
...
void SimulationThread::checkIfQueueIsFull() {
    std::unique_lock<std::mutex> runLock(this->appCtx.simulationThreadMutex);
    this->appCtx.checkIfSimulationThreadShouldRun.wait(runLock, [this] {
...
void SimulationThread::catchUpSimulation(time_point& previous) {
    int steps = 0;
    ...
    while (now >= previous && steps < this->appCtx.MAX_STEPS_PER_FRAME) {
        ...
        previous += std::chrono::duration_cast<clock::duration>(std::chrono::duration<double>(this->appCtx.mainWorld.dt));
        steps++;
    }
...
std::vector<Particle> SimulationThread::getParticleSnapshot() {
...
std::vector<uint8_t> SimulationThread::prepareSnapshotForSending(const std::vector<Particle>& particles_snapshot, nlohmann::json& metadata) {
    size_t count = particles_snapshot.size();
...
void SimulationThread::queueForSending(nlohmann::json& metadata, std::vector<uint8_t>& buffer_bytes) {
